...
class Args:
    ...
    def parse(self, argv: Sequence[str], meta: dict[str, Any]) -> tuple[dict[str, Any], CustomArgumentParser, list[str]]:
        ...
        parser.add_argument('-c', '--category', nargs=1, required=False, help="Category [movie, tv, fanres, music, book, game]", choices=['movie', 'tv', 'fanres', 'music', 'book', 'game'], dest="manual_category")
        ...
        parser.add_argument('-igdb', '--igdb', nargs=1, required=False, help="IGDB game ID", type=str, dest='igdb_manual')
        parser.add_argument('-steam', '--steam', nargs=1, required=False, help="Steam App ID or URL for IGDB lookup", type=str, dest='steam_manual')
        ...
        parser.add_argument('-plat', '--platform', '--platforms', nargs=1, required=False, help="Game platform override, e.g. PC, PS5, Switch, Xbox", type=str, dest='manual_platform')
        parser.add_argument('-gv', '--game-version', '--game_version', nargs=1, required=False, help="Game version override, e.g. v1.15", type=str, dest='game_version')
        parser.add_argument('-gsc', '--game-subcategory', '--game_subcategory', nargs=1, required=False, help="Game subcategory: full_game, full_game_dlc, dlc, update", choices=['full_game', 'full_game_dlc', 'dlc', 'update'], type=str.lower, dest='game_subcategory')
